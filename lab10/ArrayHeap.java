...
public class ArrayHeap<T> implements ExtrinsicPQ<T>
{
    ...
    private void swim(int index)
    {
        ...
        while(index > 1)
        {
            ...
            if(contents[parent].myPriority < contents[index].myPriority)
                break;
            swap(index, parent);
            index = parent;
        }
    ...
    private void sink(int index)
    {
        ...
        /** TODO: Your code here. */
        int nextIndex = minNodeOfThree(index);
        while(index != nextIndex)
        {
            swap(index, nextIndex);
            index = nextIndex;
            nextIndex = minNodeOfThree(index);
        }
    }

    /**
     * Returns the index of the node that has the lowest priority among the current node and its 2 children.
     * @param   index   The index of the current node.
     * @return          The index of the node that has the lowest priority among the current node and its 2 children.
     */
    private int minNodeOfThree(int index)
    {
        int leftChild = leftIndex(index), rightChild = rightIndex(index);
        int res = 0;
        if(leftChild > size)
            return index;
        else if(rightChild > size)
            res = (contents[index].myPriority < contents[leftChild].myPriority) ? index : leftChild;
        else
        {
            res = (contents[leftChild].myPriority < contents[rightChild].myPriority) ? leftChild : rightChild;
            res = (contents[index].myPriority < contents[res].myPriority) ? index : res;
        }
        return res;
    }
    ...
    public void changePriority(T item, double priority)
    {
        ...
        {
            if(item.equals(contents[index].myItem) == true)
                break;
        }
